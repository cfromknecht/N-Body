...
TEST( vectorTest, plusEqual ) {
  auto v = Vector3f{ 1, 2, 3 };
  auto i = Vector3f{ 1, 1, 1 };
  auto d = Vector3f{ -1, -1, -1 };
  v += i;
  ASSERT_FLOAT_EQ( v.x(), 2 );
  ASSERT_FLOAT_EQ( v.y(), 3 );
  ASSERT_FLOAT_EQ( v.z(), 4 );
  i += d;
  ASSERT_FLOAT_EQ( i.x(), 0 );
  ASSERT_FLOAT_EQ( i.y(), 0 );
  ASSERT_FLOAT_EQ( i.z(), 0 );
}
